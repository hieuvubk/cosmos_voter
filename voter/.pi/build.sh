...
# docker buildx build --tag voter --file .pi/Dockerfile --platform linux/arm64 --cache-from voter:cache --cache-to voter:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar voter
...
sudo bash -c "echo voter > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/voter.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/voter.img
